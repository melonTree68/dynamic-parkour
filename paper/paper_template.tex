...
Environment latent recovery was evaluated by obstacle family. Figure~\ref{fig:recovery_modes} compares no ELA, pure ROA-style recovery, pure teacher-student recovery, and hybrid recovery (HELA). The result is not uniform across obstacle families. Gap traversal benefits most from the teacher-student path, consistent with the need to observe moving support surfaces before contact. Explicit recovery is harmful for hurdles and tilted pads, motivating the latent suppression design in \texttt{a1\_mixed} terrain.
...
\begin{figure}[htbp]
	...
	\caption{Hybrid mixed-terrain camera distillation with and without the depth encoder scan-latent loss. The dashed line marks the shared base-RL reference.}
...
\begin{table}[htb]
	\centering
	\caption{Deployment-stage comparison of all pipelines. HELA recovers much of the baseline's degradation, while the added depth encoder loss does not improve upon HELA.}
