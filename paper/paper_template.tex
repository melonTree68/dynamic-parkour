...
Finally, the depth encoder loss result is a negative result worth reporting. Direct scan-latent matching is intuitively appealing because it gives the depth pathway an additional supervised target, but action imitation and dynamic-state supervision already provide strong gradients for the representation needed by HELA. The final policy also needs a behaviorally useful latent rather than an exact reconstruction of the teacher's internal scan representation, so the added loss may over-constrain the intermediate representation. It may still help early stability or interpretability, but in these runs it did not improve the best final waypoint result.
