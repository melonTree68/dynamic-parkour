...
RUNS = {
    ...
    "hybrid": {
        "path": "legged_gym/logs/augment-latent-hybrid-mixed-terrain/resume-from-imitate-base-15k-100/metrics.csv",
        "color": "#d62728",
    },
}
...
for label, spec in RUNS.items():
    df = keep_until(aggregate_by_checkpoint(read_metrics(spec["path"])), MAX_CHECKPOINT)
    plt.plot(
        df["checkpoint"],
