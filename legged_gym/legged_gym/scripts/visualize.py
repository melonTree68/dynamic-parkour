...
if args.histogram:
    ...
    for i in range(4):
        ...
        uniques_dec = (
            uniques[:, 0] * 8 + uniques[:, 1] * 4 + uniques[:, 2] * 2 + uniques[:, 3]
        )
...
if args.coverage:
    ...
    if args.jump:
        bins, score = state_coverage(
            features[:, :2], dim1_range=jump_xlim, dim2_range=jump_ylim
        )
    else:
        bins, score = state_coverage(
            features[:, :2], dim1_range=walk_xlim, dim2_range=walk_ylim
        )
