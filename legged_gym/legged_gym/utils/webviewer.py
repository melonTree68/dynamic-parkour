...
class WebViewer:
    def __init__(self, host: str = "127.0.0.1", port: int = 5000) -> None:
        ...
        self._app.add_url_rule(
            "/_route_stream_depth", view_func=self._route_stream_depth
        )
        self._app.add_url_rule(
            "/_route_input_event", view_func=self._route_input_event, methods=["POST"]
        )
    ...
    def _route_index(self) -> "flask.Response":
        ...
        """
        with open(
            os.path.join(LEGGED_GYM_ROOT_DIR, "legged_gym", "utils", "webviewer.html"),
            "r",
            encoding="utf-8",
        ) as file:
            template = file.read()
    ...
    def _route_stream(self) -> "flask.Response":
        ...
        """
        return flask.Response(
            self._stream(), mimetype="multipart/x-mixed-replace; boundary=frame"
        )

    def _route_stream_depth(self) -> "flask.Response":
        return flask.Response(
            self._stream_depth(), mimetype="multipart/x-mixed-replace; boundary=frame"
        )
    ...
    def attach_view_camera(self, i, env_handle, actor_handle, root_pos):
        if True:
            ...
            self._gym.set_camera_location(
                camera_handle, env_handle, gymapi.Vec3(*cam_pos), gymapi.Vec3(*root_pos)
            )
    ...
    def setup(self, env) -> None:
        ...
        for i in range(self._env.num_envs):
            ...
            self.attach_view_camera(
                i, self._envs[i], self._env.actor_handles[i], root_pos
            )
    ...
    def render(
        ...
        if self._camera_type == gymapi.IMAGE_COLOR:
            ...
            self._image = np.clip(
                1 - (self._image - minimum) / (maximum - minimum), 0, 1
            )
        ...
        if self._env.cfg.depth.use_camera:
            self._image_depth = (
                self._env.depth_buffer[self._camera_id, -1].cpu().numpy() + 0.5
            )
...
def ik(
    ...
    q = torch_utils.quat_mul(
        goal_orientation, torch_utils.quat_conjugate(current_orientation)
    )
    error = torch.cat(
        [
    ...
    if squeeze_output:
        return (
            transpose @ torch.inverse(jacobian_end_effector @ transpose + lmbda) @ error
        ).squeeze(dim=2)
    else:
        return (
            transpose @ torch.inverse(jacobian_end_effector @ transpose + lmbda) @ error
...
def print_asset_options(
    asset_options: "isaacgym.gymapi.AssetOptions", asset_name: str = ""
):
    attrs = [
        "angular_damping",
        "armature",
    ...
    for attr in attrs:
        print(
            "  |-- {}: {}".format(
                attr,
                getattr(asset_options, attr) if hasattr(asset_options, attr) else "--",
            )
        )
        # vhacd attributes
        if (
            attr == "vhacd_enabled"
            and hasattr(asset_options, attr)
            and getattr(asset_options, attr)
        ):
            vhacd_attrs = [
                "alpha",
                "beta",
