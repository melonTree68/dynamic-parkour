...
\author{
   ...
   \IEEEauthorblockA{Shanghai Jiao Tong University\\
      Email: zhijie.chen@sjtu.edu.cn}
   \and
   \IEEEauthorblockN{Shiyi Xiao}
   \IEEEauthorblockA{Shanghai Jiao Tong University\\
      Email: sjtuxsy-finance-pc@sjtu.edu.cn}
}
...
\begin{figure}[t]
   \small
   \begin{tabbing}
      \hspace{0.03in}\=\hspace{0.19in}\=\hspace{0.19in}\=\kill
      \textbf{Input:} static expert $\pi_E$, dynamic env, student $\pi_\theta$\\
      \textbf{Initialize:} teacher replay $B_E$, student replay $B_S$\\
      \textbf{for} iteration $k=0,\ldots,K-1$ \textbf{do}\\
      \> \textbf{if} $k$ is in BC warmup \textbf{then}\\
      \>\> collect observations driven by $\pi_E$ and store $(o,\pi_E(o))$ in $B_E$\\
      \> \textbf{else}\\
      \>\> collect expert-driven samples into $B_E$\\
      \>\> collect student-driven samples into $B_S$, labeled by $\pi_E$\\
      \> \textbf{end if}\\
      \> sample balanced minibatches from $B_E$ and $B_S$ when both are nonempty\\
      \> minimize $L_{\mathrm{act}}+L_{\mathrm{hist}}+L_{\mathrm{est}}$ by gradient descent\\
      \> save imitation checkpoints at the normal runner interval\\
      \textbf{end for}\\
      \textbf{return} $\pi_\theta$ for dynamic base RL fine-tuning
   \end{tabbing}
   \caption{Pseudocode for our DAgger-style imitation pretraining implementation.}
   \label{fig:dagger}
\end{figure}
