...
- `utils/plot_dynamic_terrain_layouts.py`: dynamic suite atlas generator.
- `utils/plot_original_terrain_atlas.py`: original static terrain atlas
  generator for comparison.
- `utils/check_terrain_design.py`: static checker for suite structure.
- `docs/tune.md`: concise parameter tuning guide.
...
```bash
python utils/plot_dynamic_terrain_layouts.py
```
...
```bash
python utils/check_terrain_design.py
```
